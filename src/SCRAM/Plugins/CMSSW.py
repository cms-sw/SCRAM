...
class CMSSW(object):
    ...
    def releaseArchs(self, version, default, reldir):
        ...
        xarch = None
        for line in data:
            if not prod in line: continue
            items = dict(t.split('=',1) for t in line.split(';') if '=' in t)
            if not 'architecture' in items: continue
            arch = items['architecture']
            if ('state' in items) and (items['state']=='IB'): 
                if xarch: continue
                if 'label' in items:
                    if (version == items['label']): xarch = arch
            else:
                archs.append(arch)
        if (len(archs)==0) and xarch: archs.append(xarch)
        return archs
    ...
    def getData(self, version, reldir):
        if self.data is None:
            ...
            if siteconf.get("release-checks") in ["1", "y", "yes"]:
                ...
                if e: cmd = 'curl -L -k --stderr /dev/null'
                cmd = '%s "%s"' % (cmd, url)
    ...
    def getDeprecatedDate(self, version, arch, reldir):
        ...
        for line in data:
            items = dict(t.split('=',1) for t in line.split(';') if '=' in t)
            if (not 'label' in items) or (items['label']!=version): continue
            if (not 'architecture' in items) or \
               (items['architecture'] != arch): continue
            if ('state' in items) and \
               (items['state']=='Deprecated'): return 0
            if ('deprecate_date' in items) and \
                match('^\d{8}$', items['deprecate_date']):
