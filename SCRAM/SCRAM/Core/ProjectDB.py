from __main__ import FORCED_ARCH
  ...
  return $area;
    ...
    def productionArch(self, project, version, release):
        ...
        if not archs:
            ...
            if project_module: archs = project_module.releaseArch(version, 1, release)
        arch = ""
    ...
    def listall(self, project, version, valid, all):
        ...
        xdata = self._findProjects(project, version, exact_match=False , arch=oarch, valid=valid)
        if all or ((not oarch in xdata) and (FORCED_ARCH=="")):
            for arch in self.archs:
                if arch==oarch: continue
                xdata = self._findProjects(project,version,exact_match=False,arch=arch,valid=valid,xdata=xdata)
        return xdata
    ...
    def updatearchs(self, name, version, skiparch):
        ...
        for arch in self.archs:
            ...
            data = self._findProjects(name, version, exact_match=True, arch=arch)
    ...
    def getAreaObject(self, data, arch):
        ...
        if not area.bootstrapfromlocation(loc):
            area = None
            err = "ERROR: Attempt to ressurect %s %s from $loc unsuccessful\n" %
                  (data[0], data[1])
            err += "ERROR: %s does not look like a valid release area for SCRAM_ARCH %s." %
                   (loc, arch)
...
##################################################
        ...
        for mapfile in glob('%s/*.map' % db):
            ...
            with open(mapfile, 'r') as ref:
                for line in [l.strip('\n').strip() for l in ref.readlines()]:
                    if not '=' in line: conitnue
                    proj, value = line.split('=',1)
                    proj = proj.upper()
                    self.projects[proj]=1
                    if not proj in self.DBS['uniq'][scramdb]: self.DBS['uniq'][scramdb][proj]={}
    ...
    def _findProjects(self, project='.+', version='.+', exact_match=False, arch=None, valid=False, xdata={}):
        ...
        for base in self.DBS['order']:
            for p in self.DBS['uniq'][base]:
                ...
                for x in self.DBS['uniq'][base][p]:
                    proj_dirs += glob('%s/%s' % (base, x.replace('$SCRAM_ARCH',arch)))
                for fd in proj_dirs:
                    if not isdir(fd): continue
                    if valid and (not exists(join(fd,'.SCRAM',arch,'timestamp','self'))): continue
                    ver = basename (fd)
                    if exact_match:
                        if ver == version:
